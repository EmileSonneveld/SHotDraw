...
class FloatingTextField {
   ...
   */
  def getText: String = fEditWidget.getText
  ...
  def addKeyListener(tool: TextTool) {
    fEditWidget.addKeyListener(new KeyListener() {
      override def keyTyped(e: KeyEvent) {}
      override def keyPressed(e: KeyEvent) {
        if(e.getKeyCode() == KeyEvent.VK_ENTER) {
          tool.newDrawingEditor.toolDone()
          tool.createCreationUndoActivity()
        }
      }
