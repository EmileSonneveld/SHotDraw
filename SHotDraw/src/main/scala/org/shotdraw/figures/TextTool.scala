...
class TextTool(val newDrawingEditor: DrawingEditor, prototype: Figure) extends CreationTool(newDrawingEditor, prototype) {
  /**
  ...
  def createCreationUndoActivity(): Unit = {
    newDrawingEditor.getUndoManager.pushUndo(new CreationCommand("text", getAddedFigure, newDrawingEditor).createUndoActivity)
  }

  private var myTextField: FloatingTextField = createFloatingTextField
