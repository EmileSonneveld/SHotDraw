 ...
 */
class ArrowTip(var fAngle: Double, var fOuterRadius: Double, var fInnerRadius: Double) extends AbstractLineDecoration {
  def this() {
    this(0.40, 8, 8)
  }
  ...
  def outline(x1: Int, y1: Int, x2: Int, y2: Int): Polygon = {
    val dir = math.Pi / 2 - math.atan2(x2 - x1, y2 - y1)
    outline(x1, y1, dir)
  }
